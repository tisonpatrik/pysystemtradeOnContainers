import pytest
import pandas as pd
from src.forecasting.rules.ewmac.ewmac_rule import compute_ewmac, process_data, lambda_handler

time_series_data = pd.Series([137.75, 138.00, 139.25])
volatility = pd.Series([1, 1, 1])
speed = 16
expected_ewmac = pd.Series([0, 0.005859, 0.047608])

def test_compute_ewmac_valid_input():
    
    ewmac = compute_ewmac(time_series_data, speed, volatility)
...
def test_compute_ewmac_empty_time_series():
    ...
    with pytest.raises(ValueError):
        compute_ewmac(time_series_data, speed,volatility)

...
from common.src.utils.convertors import to_series_from_dict
...
class PricesRepository:
    ...
    async def get_daily_prices_async(self, symbol: str) -> pd.Series:
        ...
        try:
            ...
            prices = to_series_from_dict(prices_data, DailyPrices, str(DailyPrices.date_time), str(DailyPrices.price))
            return prices
    ...
    async def get_denom_prices_async(self, symbol: str) -> pd.Series:
        ...
        try:
            prices_data = await self.repository.fetch_many_async(statement)
            prices = to_series_from_dict(prices_data, DenomPrices, DenomPrices.date_time, DenomPrices.price)  # type: ignore[arg-type]
            return prices
        except Exception as e:

...
class DenomPrices(DataFrameModel):
    ...
    def from_db_to_series(cls, items: list[dict]) -> pd.Series:
        ...
        data = convert_list_dicts_to_dataframe(items, str(DenomPrices.date_time), columns_names)

        validated_data = cls.validate(data)
        data_frame = cast(pd.DataFrame, validated_data)
        series = convert_dataframe_to_series(data_frame, str(DenomPrices.date_time), str(DenomPrices.price))

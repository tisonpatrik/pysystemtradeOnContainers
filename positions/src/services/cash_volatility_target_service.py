...
class CashVolTargetService:
    ...
    def get_annual_cash_vol_target(self, notional_trading_capital: float, percentage_vol_target: float) -> float:
        if not isinstance(notional_trading_capital, (int, float)) or not isinstance(
            ...
            self.logger.error("Invalid input types for notional_trading_capital or percentage_vol_target.")
            raise TypeError("Both notional_trading_capital and percentage_vol_target must be numbers.")

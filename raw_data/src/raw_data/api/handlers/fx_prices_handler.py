...
class FxPricesHandler:
    ...
    async def get_fx_prices(self, symbol: str, base_currency: str) -> pd.Series:
        self.logger.info("Fetching FX prices for symbol %s with base currency %s", symbol, base_currency)

        instrument_currency = await self.instruments_client.get_instrument_currency_async(symbol)

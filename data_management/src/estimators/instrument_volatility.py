...
class InstrumentVolEstimator:
    ...
    def get_instrument_currency_vol(
        ...
        daily_perc_vol = self.get_daily_percentage_volatility(
            multiple_prices, daily_returns_vol
        )
    ...
    def get_block_value(
        self, underlying_price: pd.Series, value_of_price_move: float
    ) -> pd.Series:
        block_value = underlying_price.ffill() * value_of_price_move * 0.01
        return block_value

    def get_daily_percentage_volatility(
        self, denom_price: pd.Series, daily_returns_vol: pd.Series
    ) -> pd.Series:

...
class RiskHandler:
    ...
    async def _get_risk_data_from_raw_file(self, model):
        ...
        if model.__tablename__ in 'robust_volatility':
            return await self.robust_volatility_service.calculate_robust_volatility_for_instrument(model)
        if model.__tablename__ in 'instrument_volatility':
            return await self.instrument_volatility_service.calculate_instrument_volatility_for_instrument(model)     

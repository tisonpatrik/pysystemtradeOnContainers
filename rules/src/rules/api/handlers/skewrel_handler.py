...
from rules.services.skewabs import SkewabsService
...
class SkewRelHandler:
    ...
    async def get_skewrel_async(self, symbol: str, speed: int, lookback: int) -> pd.Series:
        self.logger.info("Calculating SkewAbs rule for %s", symbol)
        absolute_skew_deviation = await self.raw_data_client.relative_skew_deviation_async(symbol, lookback)

...
class AggregatedReturnsForAssetClassHandler:
    ...
    async def get_aggregated_returns_for_asset_class_async(self, asset_class: str) -> pd.Series:
        ...
        try:
            ...
            async def fetch_returns(instrument_code):
                try:
                    self.logger.info(f"Fetching returns for instrument: {instrument_code.symbol}")
                    return await self.daily_vol_normalized_returns_handler.get_daily_vol_normalized_returns(instrument_code.symbol)
                except Exception as e:
                    self.logger.error(f"Error fetching returns for instrument {instrument_code.symbol}: {e}")
                    return None

            aggregate_returns_across_instruments_list = await asyncio.gather(
                *[fetch_returns(instrument_code) for instrument_code in instruments]
            )

            aggregate_returns_across_instruments_list = [returns for returns in aggregate_returns_across_instruments_list if returns is not None]

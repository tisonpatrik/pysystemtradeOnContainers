...
class WeightedForecastsWithoutMultiplierHandler:
    def __init__(
        self,
        rules_signals_client: RulesSignalsClient,
        forecasts_given_rule_list_handler: ForecastGivenRuleListHandler,
        forecast_weights_handler: ForecastWeightsHandler,
    ):
        self.logger = AppLogger.get_instance().get_logger()
        self.rules_signals_client = rules_signals_client
        self.forecast_weights_handler = forecast_weights_handler
    ...
    async def get_weighted_forecasts_without_multiplier_async(self, symbol: str) -> pd.DataFrame:
        rule_variation_list = await self.rules_signals_client.get_trading_rule_list_async()
        forecasts = await self.forecasts_given_rule_list_handler.get_forecasts_given_rule_list_async(symbol, rule_variation_list)

        smoothed_daily_forecast_weights = await self.forecast_weights_handler.get_forecast_weights_async(symbol)

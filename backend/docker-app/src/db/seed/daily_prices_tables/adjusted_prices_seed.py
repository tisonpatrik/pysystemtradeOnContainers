...
async def fetch_prices_for_symbol(async_session: sessionmaker, symbol: str) -> DataFrame:
    ...
    try:
        async with async_session() as session:
            prices_query = select(RawAdjustedPricesTable).where(RawAdjustedPricesTable.SYMBOL == symbol).order_by(RawAdjustedPricesTable.UNIX_TIMESTAMP)
...
async def seed_daily_adjusted_prices_table(async_session: sessionmaker) -> None:
    ...
    try:
        ...
        for symbol in symbols:
            ...
            if not df.empty:
                ...
                async with async_session() as session:
                    await session.bulk_insert_mappings(AdjustedPricesTable, daily_prices_df.to_dict(orient="records"))

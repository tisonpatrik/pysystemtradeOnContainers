...
def test_convert_datetime_to_unixtime_fail(mock_dataframe_for_symbol):  # Reusing the first fixture as a wrong input
...
def test_aggregate_to_day_based_prices_success(mock_dataframe_for_aggregation_success, expected_dataframe_for_aggregation_success):
    result = aggregate_to_day_based_prices(mock_dataframe_for_aggregation_success)
    
    # Check if the DataFrame is as expected
    pd.testing.assert_frame_equal(result, expected_dataframe_for_aggregation_success, check_dtype=False)

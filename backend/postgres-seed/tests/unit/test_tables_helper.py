...
def test_rename_columns_with_exception_during_rename(
     sample_dataframe
):
    new_column_names = {
        "old_name1": "new_name1",
...
def test_add_column_and_populate_it_by_value_success(
     sample_dataframe
):
...
def test_add_column_and_populate_it_by_value_failure(
     sample_dataframe
):
    with pytest.raises(SymbolAdditionError):
        ...
        add_column_and_populate_it_by_value(
            bad_dataframe, "new_column", "value"
        )
